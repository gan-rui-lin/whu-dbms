...
系统需要支持显示开启事务（begin）、提交事务（commit）、回滚事务（abort）三条事务控制语句，显式事务中只包含增删改查四种语句，不包含DDL语句。在课程提供的框架中，单条语句被包装成一个单独的事务进行执行，学生可以自行更改。在本任务中，不需要考虑并发事务的执行，测试数据中不包含并发事务。

在完成本任务之前，学生可以首先阅读项目结构文档中事务管理器的相关说明，以及代码框架中src/transaction文件夹下的代码文件。

 **提示** ：功能题目对代码框架没有限制，学生可以在原有框架的基础上进行实现，也可以修改、增添、删除数据结构及接口，或者对框架进行重构。
