...
bool LRUReplacer::victim(frame_id_t* frame_id) {
    ...
    if (frame_id == nullptr || LRUlist_.empty()) return false;
...
void LRUReplacer::pin(frame_id_t frame_id) {
    ...
    if (frame_id < 0 || static_cast<size_t>(frame_id) >= max_size_) return;
...
void LRUReplacer::unpin(frame_id_t frame_id) {
    std::scoped_lock lock{latch_};
    if (frame_id < 0 || static_cast<size_t>(frame_id) >= max_size_) return;
    if (LRUhash_.count(frame_id) != 0 || LRUlist_.size() >= max_size_) return;
