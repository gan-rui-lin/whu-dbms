...
int IxNodeHandle::lower_bound(const char *target) const {
    ...
    while (left < right) {
        ...
        if (ix_compare(get_key(mid), target, file_hdr->col_types_, file_hdr->col_lens_) < 0) left = mid + 1;
        else right = mid;
    }
    return left;
}
...
IxNodeHandle *IxIndexHandle::split(IxNodeHandle *node) {
    ...
    if (node->is_leaf_page()) {
        ...
        if (file_hdr_->last_leaf_ == node->get_page_no()) file_hdr_->last_leaf_ = new_node->get_page_no();
    } else {
        for (int i = 0; i < new_node->get_size(); ++i) maintain_child(new_node, i);
    }
    return new_node;
}
...
bool IxIndexHandle::coalesce_or_redistribute(IxNodeHandle *node, Transaction *transaction, bool *root_is_latched) {
    ...
    buffer_pool_manager_->unpin_page(neighbor->get_page_id(), true);
    if (!parent_should_delete) buffer_pool_manager_->unpin_page(parent->get_page_id(), true);
    return true;
}
...
bool IxIndexHandle::coalesce(IxNodeHandle **neighbor_node, IxNodeHandle **node, IxNodeHandle **parent, int index,
    ...
    if (!(*neighbor_node)->is_leaf_page()) {
        for (int i = old_size; i < (*neighbor_node)->get_size(); ++i) maintain_child(*neighbor_node, i);
    } else {
        ...
        if (file_hdr_->last_leaf_ == (*node)->get_page_no()) file_hdr_->last_leaf_ = (*neighbor_node)->get_page_no();
    }
    int remove_index = (*parent)->find_child(*node);
    (*parent)->erase_pair(remove_index);
    bool parent_should_delete = coalesce_or_redistribute(*parent, transaction, root_is_latched);
